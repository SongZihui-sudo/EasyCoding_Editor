		...
		//printf("key asill %d\n", ch2);
			...
			//SetPos(0,0);
                ...
                case PAGE_UP:
                    ...
                    C3.Lexical_analysis(page_arr[page_now-1]);       
                    mvprintw(page_y-2,page_x-10,"Page:%d",page_now);
					SetPos(0,1);
                ...
                case DOWN:
                    if (pos_y-  (page_now-1)*(page_y-2)>=page_arr[page_now-1].size()-1);
                        ...
                        //move(page_y,page_x-20);
                        mvprintw(page_y-2,page_x-10,"Lines:%d",pos_y);
                        pos_x = page_arr[page_now-1][pos_y-  (page_now-1)*(page_y-2)].size();
                        move(pos_y -  (page_now-1)*(page_y-2)+1,pos_x);
						refresh();
                        break;
                ...
                case LEFT:
                    if (pos_x){
                        ...
                        //cout<<pos_x<<"th";           
                        mvprintw(page_y-2,page_x-10,"%dth",pos_x);                        
                        move(pos_y -  (page_now-1)*(page_y-2)+1,pos_x);
						refresh();
                        break;
                    	//右
                case RIGHT:
                        if (pos_x>=page_arr[page_now-1][pos_y-  (page_now-1)*(page_y-2)].size()+1) pos_x = page_arr[page_now-1][pos_y-  (page_now-1)*(page_y-2)].size()+1;
                        else{
                            pos_x++;
                            last_x++;                    
                        }
